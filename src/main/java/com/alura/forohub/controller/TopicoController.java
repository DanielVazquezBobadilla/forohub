...
public class TopicoController {
    ...
    public ResponseEntity detallarTopico(@PathVariable Long id) {
        if (repository.existsById(id) && repository.findActivoById(id)){
            ...
            return ResponseEntity.ok(new ResponseDetallarTopicoDTO(topico));
        } return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No se encontro el topico");
    }
    ...
    public ResponseEntity eliminarTopico(@PathVariable Long id) {
        ...
        } return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No se encontro el tópico");
    }
